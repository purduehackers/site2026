...
function createStateTexture(gl: WebGL2RenderingContext, cols: number, rows: number, initial?: Uint8Array) {
...
export function initGliderShader(canvas: HTMLCanvasElement, opts: GliderShaderOptions): () => void {
	...
	if (!gl) {
		throw new Error('WebGL2 not available (needed for shader-based Game of Life).');
	}
